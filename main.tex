...
\section{Instruções de Formatação e Citações}\label{sec:formatacao}

\subsection{Instruções Gerais de Formatação}

O anteprojeto deve seguir o modelo SBC exigido pelo edital. Os títulos das seções devem ser apresentados em \textbf{negrito}, tamanho 13pt, alinhados à esquerda, com espaçamento adicional de 12pt antes de cada título. A numeração das seções é opcional.
...
\caption{Variáveis consideradas na avaliação de técnicas de interação}
  \begin{tabular*}{0.75\textwidth}{ccccc}
    \toprule
    \textbf{ID} & \textbf{\featN{Profissão}}	 & \textbf{\featN{Idade}}	& \textbf{\featN{Proporção}} & \textbf{\featN{Classe}} \\
    \midrule
...
\begin{itemize}
    \item Citação simples: \verb|\cite{autor2010}|;
    \item Citações múltiplas: \verb|\cite{autor2010,outroautor2015}|. 
\end{itemize}

No corpo do texto, as citações aparecerão entre colchetes, como em \cite{knuth:84}. Recomenda-se citar apenas trabalhos efetivamente utilizados na fundamentação teórica, na discussão da metodologia ou na análise dos resultados.

A lista de referências deve seguir o estilo definido pelo pacote \texttt{sbc-template}, sendo automaticamente formatada pelos comandos:
